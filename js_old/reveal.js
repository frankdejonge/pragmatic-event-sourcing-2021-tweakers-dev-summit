...
export default function( revealElement, options ) {
	...
	let config = {},
		...
		// Flags if reveal.js_old is loaded (has dispatched the 'ready' event)
		ready = false,
	...
	function initialize( initOptions ) {
		...
		// 1. Default reveal.js_old options
	...
	/**
	 * Encase the presentation in a reveal.js_old viewport. The
	...
	/**
	 * Starts up reveal.js_old by binding input events and navigating
	 ...
	 * reveal.js_old with showHiddenSlides set to true.
	 */
	 ...
	 * possible to call all reveal.js_old API methods from another
	 * window. For example:
	 *
				...
				if( typeof data === 'string' && data.charAt( 0 ) === '{' && data.charAt( data.length - 1 ) === '}' ) {
					...
					if( data.method && typeof Reveal[data.method] === 'function' ) {
						...
						if( POST_MESSAGE_METHOD_BLACKLIST.test( data.method ) === false ) {
							...
							console.warn( 'reveal.js_old: "'+ data.method +'" is is blacklisted from the postMessage API' );
						}
	...
	function configure( options ) {
		...
		// Abort if reveal.js_old hasn't finished loading, config
	...
	/**
	 * Adds a listener to one of our custom reveal.js_old events,
	 * like slidechanged.
	 */
	...
	/**
	 * Unsubscribes from a reveal.js_old event.
	 */
	...
	function dispatchEvent({ target=dom.wrapper, type, data, bubbles=true }) {
		...
		if( target === dom.wrapper ) {
			// If we're in an iframe, post each reveal.js_old event to the
	...
	/**
	 * Updates reveal.js_old to keep in sync with new slide attributes. For
	 * example, if you add a new `data-background-image` you can call
	 * this to have reveal.js_old render the new background image.
	 *
	...
	const API = {
		...
		// Checks if reveal.js_old has been loaded and is ready for use
		isReady: () => ready,
		...
		// Returns reveal.js_old DOM elements
